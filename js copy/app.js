...
document.addEventListener('DOMContentLoaded', async function () {
   ...
   document.addEventListener("click", function (e) {
      if (e.target && e.target.id === "openObsModalBtn") {
         if (obsModal) {
            ...
            // Sätt automatiskt dagens datum i datumväljaren
            document.getElementById("obsDatum").value = new Date().toISOString().split('T')[0];

            // 🔥 NYTT: Sätt automatiskt nuvarande lokal tid (HH:MM) i tidsväljaren
            const nu = new Date();
            const nuvarandeTid = nu.toTimeString().split(' ')[0].substring(0, 5); // Ger t.ex. "14:25"
   ...
   if (addObsForm) {
      addObsForm.addEventListener("submit", async (e) => {
         ...
         // 🔥 NYTT: Hämta klockslaget från det nya fältet
         const tid = document.getElementById("obsTid").value;

         // 🔥 UPPDATERAD: Skicka med "tid" som ett argument till din funktion i database.js
