...
/* 
   ...
   const nyaDatasets = valdaArtIds.map(artId => {
      ...
      return {
         label:` color: #ffffff, ${artInfo.namn} (${totaltAntal} st)`,
         data: punkterData,
...
export function uppdateraGraf(valdaArtIds = [], filtreradData = []) {
   ...
   valdaArtIds.forEach((artId, index) => {
      ...
      // 4. SÄTT KÄLLAN SISS: Nu kan webbläsaren börja leta, and har koll på din onerror!
      ikonImg.src = `images/svg/${slug}.svg`;
 ...
 // 🌟 KÄNN AV LÄGET DIREKT (Utgår från ljust läge om inget annat anges)
   const isDark = document.body.classList.contains("dark-mode") || 
                  document.documentElement.dataset.theme === "dark";
   
   // Mörkgrå (#333333) för ljust läge, kritvit (#ffffff) för mörkt läge
   const nuvarandeFarg = isDark ? '#ffffff' : '#333333';

   // Sätt standardvärden
   Chart.defaults.color = nuvarandeFarg;
   Chart.defaults.plugins.legend.labels.color = nuvarandeFarg;
   ...
   if (trendsChart) {
      ...
      // Sätt färg och bold vid uppdatering
      trendsChart.options.plugins.legend.labels.color = nuvarandeFarg;
      trendsChart.options.plugins.legend.labels.font = {
         weight: 'bold',
         size: 14,
         family: 'sans-serif'
      };
      
      if (trendsChart.options.scales.x?.ticks) trendsChart.options.scales.x.ticks.color = nuvarandeFarg;
      if (trendsChart.options.scales.y?.ticks) trendsChart.options.scales.y.ticks.color = nuvarandeFarg;
      ...
      trendsChart = new Chart(ctx, {
         ...
         options: {
            ...
            scales: {
               x: { 
                  grid: { display: false }, 
                  ticks: { maxTicksLimit: 10, color: nuvarandeFarg } 
               },
               y: { 
                  beginAtZero: true, 
                  ticks: { precision: 0, color: nuvarandeFarg } 
               }
            }
         }
      });
