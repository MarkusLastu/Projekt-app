...
/* export function uppdateraKartaEfterFilter() {
   ...
   let filtrerade = allaObservationer.filter(obs => {
      ...
      const filterMin = document.getElementById('dateMin')?.value;
      const filterMax = document.getElementById('dateMax')?.value;
      const tidsMatch = (!filterMin || faktisktDatum >= filterMin) && (!filterMax || faktisktDatum <= filterMax);

      const skaBehallas = artMatch && lanMatch && kommunMatch && tidsMatch;

      if (skaBehallas) {
         // Plocka namnet från vår ordlista vi skapade i steg 2
         obs.artNamn = artNamnOrdlista[parseInt(nuvarandeArtId)] || "Okänt djur";
      }
   ...
   console.log(`🎯 Matchningar hittade efter filter: ${filtrerade.length}`);
   mapModul.taEmotOchRitaObservationer(filtrerade);
} */
...
/* export function uppdateraKartaEfterFilter() {
   ...
   // 1. Hämta alla aktuella värden från GUI:t
   ...
   // Hämta exakta datum från våra nya textrutor/kalendrar
   const filterMin = document.getElementById('dateMin')?.value;
   const filterMax = document.getElementById('dateMax')?.value; */
/* // Reglera slider-värden
const minVal = parseInt(sliderMin.value);
const maxVal = parseInt(sliderMax.value);
if (periodText) periodText.textContent = `${indexTillText(minVal)} till ${indexTillText(maxVal)}`; */

/* // 🔥 KRAV: Om inga djur är ikryssade överhuvudtaget -> Töm kartan direkt och stoppa!
if (valdaArtIds.length === 0) {
   mapModul.taEmotOchRitaObservationer([]);
   ui.skapaLoggar(uppdateraKartaEfterFilter, 'ok', `🔍 Visar 0 observationer (inga djur valda).`, observationStatus);
   return;
}
*/
...
/* let filtrerade = allaObservationer.filter(obs => {
   ...
   // B. Hitta vilken region denna specifika observation tillhör utifrån dess ID
   const kommunInfo = allRegionData.find(r => r.kommun_id === obs.Kommun_id);

   // C. Kolla om län och kommun matchar de textsträngar som är valda i menyerna
   const lanMatch = (valtLan === "alla" || (kommunInfo && kommunInfo.lan_namn === valtLan));
   const kommunMatch = (valtKommun === "alla" || valtKommun === "" || (kommunInfo && kommunInfo.kommun_namn === valtKommun));

   // 🔥 RÄTTNING HÄR: Säkra upp om fältet heter "Datum" eller "datum" i objektet
   const faktisktDatum = obs.Datum || obs.datum;

   // D. Tidsfilter baserat på det säkrade datumet
   const tidsMatch = (!filterMin || faktisktDatum >= filterMin) && (!filterMax || faktisktDatum <= filterMax);

   return artMatch && lanMatch && kommunMatch && tidsMatch;
});
*/


/* // D. Tidsfilter baserat på exakta datum i kalenderfälten
const filterMin = document.getElementById('dateMin').value;
const filterMax = document.getElementById('dateMax').value; */

// obs.Datum är t.ex. "2026-04-12"
/* const tidsMatch = (!filterMin || obs.Datum >= filterMin) && (!filterMax || obs.Datum <= filterMax); */
/* const obsIndex = hamtaIndexFranDatum(obs.Datum);
const tidsMatch = (obsIndex >= minVal && obsIndex <= maxVal); */


// 3. Uppdatera grafen (Vi räknar om till index temporärt för grafens x-axel om det behövs)
/* const minIndex = filterMin ? hamtaIndexFranDatum(filterMin) : 0;
const maxIndex = filterMax ? hamtaIndexFranDatum(filterMax) : 20;
uppdateraGraf(valdaArtIds, minIndex, maxIndex);

// 4. Skicka till kartan
const punkterTillKartan = filtrerade
   .filter(obs => (obs.Latitude || obs.latitude) && (obs.Longitude || obs.longitude))
   .map(obs => ({
      lat: parseFloat(obs.Latitude || obs.latitude),
      lon: parseFloat(obs.Longitude || obs.longitude),
      artNamn: obs.ArtNamn || obs.artNamn,
      datum: obs.Datum || obs.datum, // 🔥 Säkrat för kartan!
      tid: obs.Tid || obs.tid
   }));
...
if (observationStatus) {
   ui.skapaLoggar(uppdateraKartaEfterFilter, 'ok', `🔍 Visar ${filtrerade.length} observationer.`, observationStatus);
}
} */
...
/* export function uppdateraKartaEfterFilter() {
   ...
   console.log("Check 1: Finns allRegionData?", allRegionData.length > 0);
   console.log("Check 2: Första observationen innehåller:", allaObservationer[0]);

   // 1. Hämta alla aktuella värden från GUI:t
   const valtLan = document.getElementById('lanFilter').value;
   const valtKommun = document.getElementById('obsKommun').value; // ID från HTML
   const ikryssadeCheckboxar = document.querySelectorAll('.art-checkbox:checked');
   const valdaArtIds = Array.from(ikryssadeCheckboxar).map(cb => parseInt(cb.value));
   ...
   // 1. Filtrera observationer
   // Vi sparar resultatet i variabeln 'filtrerade'
   let filtrerade = allaObservationer.filter(obs => {
      const kommunInfo = allRegionData.find(r => r.kommun_id === obs.Kommun_id);
      const matchLan = (valtLan === "alla" || (kommunInfo && kommunInfo.lan_namn === valtLan));
      const matchKommun = (valtKommun === "" || valtKommun === "alla" || obs.Kommun_id == valtKommun);
      return matchLan && matchKommun;
   });

   console.log(`Visar ${filtrerade.length} observationer.`);

   // 3. Uppdatera grafen
   uppdateraGraf(valdaArtIds, minVal, maxVal);

   // 4. Skicka till kartan
   // HÄR VAR FELET: Ändra 'filtreradData' till 'filtrerade'
   const punkterTillKartan = filtrerade
   ...
   // HÄR OCKSÅ: Ändra 'filtreradData' till 'filtrerade'
   ui.skapaLoggar(uppdateraKartaEfterFilter, 'ok', `🔍 Visar ${filtrerade.length} observationer.`, observationStatus);
} */
...
/* document.addEventListener('DOMContentLoaded', async function () {
   ...
   if (saveAddBtn) {
      saveAddBtn.addEventListener("click", async () => {
         ...
         // Stäng rutan och ladda om listan på skärmen
         ui.closeAddModal();
         await loadProjektStatus();
   ...
   // Lägg till dessa två rader HÄR (och se till att de inte är utkommenterade):
   ...
   if (datePresetSelect && dateMinInput && dateMaxInput) {
      datePresetSelect.addEventListener('change', function () {
         ...
         let startStr = "2016-01-01"; // Standard för "Alla år"
         ...
         if (val === "30days") {
            ...
            // Låt användaren ändra manuellt i rutorna utan att skriva över dem direkt
            return;
         }
   ...
   /* // Testar datum istället för slide
   const dateMinInput = document.getElementById('dateMin');
   const dateMaxInput = document.getElementById('dateMax'); */
...
/* if (dateMinInput && dateMaxInput) {
   ...
   // Räkna ut 30 dagar bakåt
   const startDatum = new Date();
   startDatum.setDate(idag.getDate() - 30);
   const startStr = startDatum.toISOString().split('T')[0];

   // Tilldela värdena till input-fälten
   dateMinInput.value = startStr;
   dateMaxInput.value = idagStr;

   // Tvinga dropdownen att visa "Senaste 30 dagarna" vid start
   if (datePresetSelect) {
...
document.querySelectorAll('.preset-btn').forEach(btn => {
   btn.addEventListener('click', function () {
      const nu = new Date();
      const maxIndex = hamtaIndexFranDatum(nu.toISOString().split('T')[0]); // Dagens index (2026)
      let minIndex = maxIndex; // Standardinställning

      const days = this.getAttribute('data-days');
      const preset = this.getAttribute('data-preset');

      if (days) {
         // Räkna ut datumet X dagar bakåt i tiden
         const startDatum = new Date();
         ...
         // Brunstperioden för älg är generellt sept-okt. 
         // Om vi vill visa t.ex. hösten 2025 (H2), sätter vi fasta index för det,
         // eller räknar ut det baserat på nuvarande år.
         const nuvarandeAr = nu.getFullYear();
         // Formel: (År - 2016) * 2 + (isH2 ? 1 : 0)
         // Vi sätter den till H2 (hösten) för föregående eller nuvarande år:
         minIndex = (nuvarandeAr - 2016) * 2 + 1; // H2 för i år
         // Om vi vill att den ska sträcka sig över hela året för att säkra upp:
         // minIndex = maxIndex - 1; 
      } else if (preset === 'all') {
         minIndex = 0; // Början av slidern (2016 H1)
      }

      // Säkerställ att minIndex inte blir mindre än 0
      if (minIndex < 0) minIndex = 0;

      // Uppdatera HTML-sliderns faktiska värden på skärmen
      sliderMin.value = minIndex;
      sliderMax.value = maxIndex;

      // Eftersom slidern har ändrats mekaniskt måste vi köra din filtrering manuellt
      uppdateraKartaEfterFilter();
   });
});
...
/*  document.getElementById('lanFilter').addEventListener('change', function () {
    const valtLan = this.value;
    const kommunFilter = document.getElementById('kommunFilter');

    // Nollställ kommunväljaren
    ...
    // Hämta kommuner från allRegionData baserat på valt län
    ...
    kommunerILanet.forEach(k => {
       ...
       opt.value = k.kommun_namn; // Vi sparar namnet som värde för filtreringen
 ...
 document.getElementById('kommunFilter').addEventListener('change', uppdateraKartaEfterFilter);
...
/* if (dateMinInput && dateMaxInput) {
   dateMinInput.addEventListener('change', uppdateraKartaEfterFilter);
   dateMaxInput.addEventListener('change', uppdateraKartaEfterFilter);
}
...
document.querySelectorAll('.preset-btn').forEach(btn => {
   btn.addEventListener('click', function () {
      ...
      let startStr = "2016-01-01"; // Standard för "Visa allt"
      ...
      if (range === "7" || range === "30") {
         ...
         // Sätt t.ex. höstperioden för senaste året (september till oktober)
...
async function laddaToppmenyVader() {
   ...
   // 🔥 FIXA TIDSZONS-BUGGEN: Hämta lokalt datum istället för UTC-datum
   const nu = new Date();
   const ar = nu.getFullYear();
   const manad = String(nu.getMonth() + 1).padStart(2, '0');
   const dag = String(nu.getDate()).padStart(2, '0');

   const idag = `${ar}-${manad}-${dag}`; // Blir garanterat rätt lokalt datum (YYYY-MM-DD)
...
// Kör en första filtrering så kartan ritas ut direkt när datan kommit in från Supabase
setTimeout(() => {
   uppdateraKartaEfterFilter();
}, 1000); // Ger databasladdningen en liten sekund att landa först
...
/* if (document.getElementById("wikiStatus")) {
   ...
   fetch('https://api.gbif.org/v1/species/match?name=Lupus')
      .then(res => res.json())
      .then(data => {
         // ✅ När vi får svar, flagga som klart!
...
*/
// Testkör Freesound (sök på Canis lupus)
...
// 2. Kör funktionen asynkront och hantera logg-statusen baserat på resultatet
api.hamtaLjudUrl('Canis lupus familiaris')
   .then(mp3Url => {
      if (mp3Url) {
         // ✅ API:et svarade och vi fick en giltig ljudlänk!
         ui.skapaLoggar('Testkörning Freesound', 'ok', 'Freesound svarade OK! Hittade ljudfil.', freesoundLogElement);
      } else {
         // ⚠️ Funktionen returnerade null (fångades i dess interna catch)
         ui.skapaLoggar('Testkörning Freesound', 'fel', 'Freesound misslyckades (inga resultat eller felaktig länk).', freesoundLogElement);
      }
   })
   .catch(err => {
      // ❌ Om något helt oväntat kraschade utanför funktionens egna try/catch
...
document.getElementById("arterFilterGroup")?.addEventListener("click", async (e) => {
   // Kolla om det var just en ljudknapp (.sound-btn) man klickade på
   if (e.target.classList.contains("sound-btn")) {
      e.preventDefault();
      e.stopPropagation();

      const knapp = e.target;
      // Hämtar det latinska namnet direkt från "data-latin" som vi satte i ui.js!
      const latinName = knapp.getAttribute("data-latin");

      if (!latinName) {
         console.warn("Knappen saknar latinskt namn (data-latin).");
         return;
      }
...
// Anropar din Freesound-funktion via din api-modul
const audioUrl = await api.hamtaLjudUrl(latinName);

if (audioUrl) {
   // Om ett annat ljud spelas, stäng av det först
...
/*  // === LOGIK FÖR ATT RAPPORTERA NY OBSERVATION ===
 ...
 document.addEventListener("click", function (e) {
    ...
    // Enkel sammanslagning: Kolla openBtn och obsModal i samma if-sats
    if (openBtn && obsModal) {
       obsModal.classList.remove("hidden");

       // FIXA TIDSZONS-BUGGEN: Hämta lokal tid istället för UTC
       const nu = new Date();
       const ar = nu.getFullYear();
       const manad = String(nu.getMonth() + 1).padStart(2, '0'); // Månader börjar på 0
       const dag = String(nu.getDate()).padStart(2, '0');

       // Sätt automatiskt lokalt datum (YYYY-MM-DD)
       document.getElementById("obsDatum").value = `${ar}-${manad}-${dag}`;

       // Sätt automatiskt nuvarande lokal tid (HH:MM)
       const nuvarandeTid = nu.toTimeString().split(' ')[0].substring(0, 5);
       document.getElementById("obsTid").value = nuvarandeTid;
    }
 }); */
...
if (addObsForm) {
   addObsForm.addEventListener("submit", async (e) => {
      ...
      // Inuti addObsForm submit-eventet:
      e.preventDefault();

      // 1. Hämta de dolda GBIF-värdena istället för en rullista
      ...
      // Validera att man faktiskt valde något från GBIF-listan
      ...
      // 🔥 MAGIN HÄNDER HÄR: Hämta befintligt ID eller skapa ett nytt i tabellen 'arter' helt live!
...
/*     // Nu skickar vi med det korrekta artId precis som förr! Databasen märker ingen skillnad!
    console.log("✈️ APP.JS SKICKAR:", { artId, datum, lat, lon, kommunId, tid });
    const lyckades = await database.insertObservation(artId, datum, lat, lon, kommunId, tid);

    if (lyckades) {
       obsModal.classList.add("hidden");
       addObsForm.reset();
       // Glöm inte att tömma de dolda fälten också vid reset
...
// Samla in och formatera alla värden prydligt på ett ställe
...
// 🛑 VALIDERING 1: Säkra upp att man valt ett djurslag
if (isNaN(artId)) {
   ui.skapaLoggar('skickaObservation', 'fel', 'Inget djurslag valt.');
   alert("Vänligen välj ett djurslag ur listan.");
   return;
}

// 🛑 VALIDERING 2: Säkra upp att man valt en kommun
if (isNaN(kommunId)) {
   ui.skapaLoggar('skickaObservation', 'fel', 'Ingen kommun vald.');
   alert("Vänligen välj en kommun ur listan.");
   return;
}

// Skicka datan till database.js
const lyckades = await database.insertObservation(artId, datum, lat, lon, kommunId, tid);

if (lyckades) {
   obsModal.classList.add("hidden");
   addObsForm.reset(); // 🔥 Tömmer formuläret så det är rent till nästa gång
   mapModul.clearObservationMarkers();
   uppdateraKartaEfterFilter();
   alert("Observationen har sparats i databasen! 🎉");
} else {
...
/* 
   ...
   if (artSökInput && artFörslagDiv) {
      artSökInput.addEventListener("input", async (e) => {
         ...
         try {
            // 🔥 NU MED BACKBONE-NYCKEL & VI HÄMTAR 20 ST FÖR ATT KUNNA JÄMFÖRA
 ...
 */
// ✨ SMART BEHANDLING: Gräv ut svenska namn och sätt relevans-poäng
/*      let bearbetadeArter = rawFörslag.map(art => {
        let svensktNamn = "";

        // Leta efter det svenska namnet i listan
        if (art.vernacularNames && art.vernacularNames.length > 0) {
           ...
           if (svenskMatch) {
              svensktNamn = svenskMatch.vernacularName;
           }
        }
        if (!svensktNamn && art.vernacularName) {
           svensktNamn = art.vernacularName;
        }

        // Starta på 0 poäng (t.ex. om det bara matchar en forskare/latin)
        let poäng = 0;

        if (svensktNamn) {
           poäng += 10; // Ge massor av poäng om arten överhuvudtaget har ett svenskt namn!

           // Extra bonuspoäng om det svenska namnet startar på eller innehåller det vi sökte på
           if (svensktNamn.toLowerCase().includes(sökord.toLowerCase())) {
              poäng += 5;
           }
           if (svensktNamn.toLowerCase().startsWith(sökord.toLowerCase())) {
              poäng += 5;
           }
        }

        // Snygga till startbokstaven om vi hittade ett svenskt namn
        if (svensktNamn) {
           svensktNamn = svensktNamn.charAt(0).toUpperCase() + svensktNamn.slice(1);
        } else {
           // Fallback om inget svenskt namn finns alls i GBIF
           svensktNamn = art.canonicalName || sökord;
        } */
/* 
               const latinsktNamn = art.canonicalName || art.scientificName;
            ...
            // 🏆 Sortera listan så att de med högst poäng (riktiga svenska matchningar) hamnar överst!
            bearbetadeArter.sort((a, b) => b.poäng - a.poäng);

            // Skär ner listan till de 5 absolut bästa resultaten efter sorteringen
            const deFemBästa = bearbetadeArter.slice(0, 5);

            // Rita ut de rättsorterade förslagen
